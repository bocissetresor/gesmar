        ...
        <div class="row">
            <div class="col-md-12 col-sm-12">
                <div class="x_panel">
                    ...
                    <div class="x_content">
                        <form method="POST" action="{{ url('ordonnancement_buro2/'. $contrats->id) }}" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="field item form-group">
                                ...
                                <div class="col-md-6 col-sm-6">
                                    <input class="form-control" class='optional' name="loyer" data-validate-length-range="5,15" type="number" value="{{ $contrats->emplacements->sum('pas_porte') }}" readonly /></div>
                            </div>
                            <div class="field item form-group">
                                <label class="col-form-label col-md-3 col-sm-3  label-align">Somme Totale :<span class="required">*</span></label>
                                <div class="col-md-6 col-sm-6">
                                    <input class="form-control" id="total" class='optional' name="loyer" data-validate-length-range="5,15" type="number" value="{{ $contrats->emplacements->sum('loyer')+$contrats->emplacements->sum('pas_porte') }}" readonly /></div>
                            </div>
                            
                                {{-- <button type="button" class="add_field_button">Ajouter un ordonancement</button>
                                <div id="field"><input type="date" min="{{ now()->toDateString('Y-m-d') }}" name="date_ordonnacement[]" required>
                                <input type="number" name="somme_ordonnacement[]" min="{{ 0 }}" max="{{ $contrats->emplacements->sum('loyer')+$contrats->emplacements->sum('pas_porte') }}" required></div>
                                 --}}
                                <div class="input_fields_wrap" style="width:100%; width: 400px;margin: auto; ">
                                    ...
                                    <input type="number" class="form-control mb-3" name="somme_ordonnacement[]" min="{{ 0 }}" max="{{ $contrats->emplacements->sum('loyer')+$contrats->emplacements->sum('pas_porte') }}" required>
                                </div>
