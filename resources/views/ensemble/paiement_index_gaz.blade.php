        ...
        <div class="row">
            <div class="col-md-12 col-sm-12">
                <div class="x_panel">
                    ...
                    <form action="{{ url('paiement_index_gaz_payer/'. $gazs->id) }}" method="POST">
                        @csrf
                        <div class="field item form-group">
                            <label class="col-form-label col-md-3 col-sm-3  label-align">Code Compteur GAZ:<span class="required">*</span></label>
                            <div class="col-md-6 col-sm-6">
                                <input class="form-control" value="{{ $gazs->code }}" class='optional' name="" data-validate-length-range="5,15" type="text" readonly/></div>
                        </div>

                            {{-- Code Locataire : <input type="text" name="code_locataire" value="{{ $gazs->contrat->code_locataire }}" readonly><br> --}}
                            <hr>
                            ...
                            <tbody>
                                @foreach ($gazs->ordonnacement_gazs as $ordonnacement_gaz)
                                <tr>
                                <td><input type="date" class="form-control variable-field quantity" name="" value="{{ $ordonnacement_gaz->date_ordonnacement }}" readonly/></td>
                                <td><input type="number" class="form-control variable-field costpriceunit" name="" value="{{ $ordonnacement_gaz->somme_ordonnacement }}" readonly/></td>
                            ...
                            <tbody>
                                @foreach ($gazs->ordonnacement_gazs as $ordonnacement_gaz)
                                @if ($ordonnacement_gaz->date_ordonnacement== date('Y-m-d') || $ordonnacement_gaz->date_ordonnacement < date('Y-m-d'))

                                @if ( ($gazs->index_fn - $gazs->index_dbt)*1022  > 0)



                                <tr>
                                    <td><input type="number" class="form-control variable-field" name="id_ordonnacement" value="{{ $ordonnacement_gaz->id }}" readonly></td>
                                    ...
                                    <td><input type="number" class="form-control variable-field quantity" value="{{ (($gazs->index_fn - $gazs->index_dbt)*1022 + $gazs->somme_restant) }}" readonly /></td>
                                    <td><input type="number" class="form-control variable-field costpriceunit" name="somme_payer" value="{{ $ordonnacement_gaz->somme_ordonnacement }}" readonly/></td>
